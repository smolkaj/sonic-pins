...
TEST(FourwardDvaasTest, UpstreamDvaasValidation) {
  ...
  // Install forwarding entries on the SUT. The copy-to-CPU entry ensures DVaaS
  // can observe punted packets on the SUT's StreamChannel.
  {
    ASSERT_OK_AND_ASSIGN(
        auto sut_session,
  ...
  *spec.bmv2_config.mutable_p4info() =
  ...
  // Verify packets actually traversed the link between SUT and control switch.
  // Without this, a broken PacketBridge (or broken packet IO) would cause both
  // prediction and SUT to produce zero outputs — a silent "DROP == DROP" pass.
  EXPECT_GT(bridge.PacketsForwarded(), 0)
      << "PacketBridge forwarded zero packets. This likely indicates broken "
         "packet IO or a misconfigured pipeline — both sides silently dropped.";
  ...
  if (result.ok()) {
    ...
    // WORKAROUND: 4ward's PacketIn metadata encodes port-typed fields (e.g.
    // target_egress_port) using the raw dataplane port number when no P4RT
    // reverse mapping exists (the CPU port is never forward-allocated). pdpi
    // rejects these as invalid UTF-8 for sdn_string types. This is a 4ward
    // bug, not a sonic-pins issue.
    // TODO(4ward): Fix PacketIn port metadata encoding for unmapped ports.
    EXPECT_THAT(result.status().message(),
                testing::HasSubstr("target_egress_port"))
        << "Unexpected DVaaS failure: " << result.status().message();
    LOG(WARNING) << "DVaaS validation hit known 4ward limitation: "
